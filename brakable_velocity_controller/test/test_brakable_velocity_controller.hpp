...
#ifndef TEST_BRAKABLE_VELOCITY_CONTROLLER_HPP_
#define TEST_BRAKABLE_VELOCITY_CONTROLLER_HPP_
...
using ControllerStateMsg = brakable_velocity_controller::BrakableVelocityController::ControllerStateMsg;
using ControllerCommandMsg = brakable_velocity_controller::BrakableVelocityController::ControllerReferenceMsg;
using ControllerModeSrvType = brakable_velocity_controller::BrakableVelocityController::ControllerModeSrvType;
...
class TestableBrakableVelocityController : public brakable_velocity_controller::BrakableVelocityController
{
  FRIEND_TEST(BrakableVelocityControllerTest, all_parameters_set_configure_success);
  FRIEND_TEST(BrakableVelocityControllerTest, activate_success);
  FRIEND_TEST(BrakableVelocityControllerTest, reactivate_success);
  FRIEND_TEST(BrakableVelocityControllerTest, test_feedforward_mode_service);
  FRIEND_TEST(BrakableVelocityControllerTest, test_update_logic_feedforward_off);
  FRIEND_TEST(BrakableVelocityControllerTest, test_update_logic_feedforward_on);
  FRIEND_TEST(BrakableVelocityControllerTest, test_update_logic_chainable_feedforward_off);
  FRIEND_TEST(BrakableVelocityControllerTest, test_update_logic_chainable_feedforward_on);
  FRIEND_TEST(BrakableVelocityControllerTest, subscribe_and_get_messages_success);
  FRIEND_TEST(BrakableVelocityControllerTest, receive_message_and_publish_updated_status);

public:
  controller_interface::CallbackReturn on_configure(
    const rclcpp_lifecycle::State & previous_state) override
  {
    return brakable_velocity_controller::BrakableVelocityController::on_configure(previous_state);
  }
    ...
    const rclcpp_lifecycle::State & previous_state) override
  {
    ...
    return brakable_velocity_controller::BrakableVelocityController::on_activate(previous_state);
  }
...
class BrakableVelocityControllerFixture : public ::testing::Test
{
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_ = command_publisher_node_->create_publisher<ControllerCommandMsg>(
      "/test_brakable_velocity_controller/reference", rclcpp::SystemDefaultsQoS());
    ...
    feedforward_service_client_ = service_caller_node_->create_client<ControllerModeSrvType>(
      "/test_brakable_velocity_controller/set_feedforward_control");
  }
...
protected:
  void SetUpController(const std::string controller_name = "test_brakable_velocity_controller")
  {
    ASSERT_EQ(
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_brakable_velocity_controller/controller_state", 10, subs_callback);
...
protected:
  ...
  std::unique_ptr<TestableBrakableVelocityController> controller_;
...
#endif  // TEST_BRAKABLE_VELOCITY_CONTROLLER_HPP_
